...
class HomeController extends Controller
{
    ...
    {
        return view('frontend.track_resi');
    }
    ...
    public function shop_open(Request $req)
    {
        $update = User::where('id', Auth::id())->update(['status' => 'active']);

        if($update)
        {
            $update2 = Product::where('user_id', Auth::id())->update(['status' => 'active']);
            if($update2)
            {
    ...
    public function shop_close(Request $req)
    {
        $update = User::where('id', Auth::id())->update(['status' => 'close']);

        if($update)
        {
            $update2 = Product::where('user_id', Auth::id())->update(['status' => 'close']);
            if($update2)
            {
                return redirect()->back();
            }
