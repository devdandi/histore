...
class CheckoutController extends Controller
{
    ...
    public function checkout_done($order_id, $payment)
    {
        ...
        for($i = 0; $i < count(session('cart_new')); $i++)
        {
            $order->orderDetails[$i]->shipping_cost = session('cart_new')[$i]['shipping_costs'];
            $order->orderDetails[$i]->service = session('cart_new')[$i]['service'];
            $order->orderDetails[$i]->logistics = session('cart_new')[$i]['sent_by'];
            $order->orderDetails[$i]->save();
        }
    ...
    public function store_shipping_info(Request $request)
    {
        ...
        if (Auth::check()) {
            ...
            $explode = explode('|', $address->city);
            $explode_province = explode("|", $address->province);
            $data['name'] = $address->name;
            $data['email'] = Auth::user()->email;
            $data['address'] = $address->address;
            $data['country'] = $address->country;
            $data['district'] = $address->district;
            $data['province'] = $explode_province[1];
            $data['city'] = $explode[0];
            $data['type'] = $explode[1];
        ...
        if(Session::has('cart') && count(Session::get('cart')) > 0){
            ...
            $cart = $cart->map(function ($object, $key) use ($request) {
                if(\App\Product::find($object['id'])->added_by == 'admin'){
                    ...
                    if($request['shipping_type'.\App\Product::find($object['id'])->user_id] == 'home_delivery'){
            ...
            foreach($request->shipping_type as $types)
            {
                $explode = explode('|', $types);
                $shipping += $explode[2];
                // array_merge($arr, array('shipping_costs' => $explode[2],'service' => $explode[1],'sent_by' => $explode[0]));
                array_push($arr,['sent_by' => $explode[0],'service' => $explode[1],'shipping_costs' => $explode[2]]);
            }
           ...
           session()->put('cart_new', $arr);
        //    dd(session()->get('cart_new'));
            return view('frontend.payment_select', compact('total'));
        }
        else {
    ...
    public function update_shipping_on_cart(Request $req)
    {
        session()->put('cart_new', []);
        $arr = array();
        $ex = explode('|', $req->data);
        ...
        session()->push('cart_new', $arr);

        return session()->get('cart_new');
    }
    public function get_shipping_on_cart()
    {
        // return session('cart');
    }
}
