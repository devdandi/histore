    ...
    <div class="pt-4">
        <ul class="process-steps clearfix">
            <li @if($status == 'pending') class="active" @else class="done" @endif>
                <div class="icon"></div>
				<div class="title"><img src="{{ ('public/uploads/icon/orderaccept.png') }}" ></div>                
            </li>
            <li @if($status == 'on_review') class="active" @elseif($status == 'on_delivery' || $status == 'delivered') class="done" @endif>
                <div class="icon"></div>
                <div class="title"><img src="{{ ('public/uploads/icon/packing.png') }}" ></div>
            </li>
            <li @if($status == 'on_delivery') class="active" @elseif($status == 'delivered') class="done" @endif>
                <div class="icon"></div>
                <div class="title"><img src="{{ ('public/uploads/icon/ondelivery.png') }}" ></div>
            </li>
            <li @if($status == 'delivered') class="done" @endif>
                <div class="icon"></div>
                <div class="title"><img src="{{ ('public/uploads/icon/itemrecieved.png') }}" ></div>
            </li>
        </ul>
    </div>
    <div class="row mt-5">
        <div class="offset-lg-4 col-lg-4 col-sm-6">
            <div class="form-inline">
                <select class="form-control selectpicker form-control-sm"  data-minimum-results-for-search="Infinity" id="update_delivery_status">
                    <option value="pending" @if ($status == 'pending') selected @endif>{{ translate('Order Accepted')}}</option>
                    <option value="on_review" @if ($status == 'on_review') selected @endif>{{ translate('Packing')}}</option>
                    <option value="on_delivery" @if ($status == 'on_delivery') selected @endif>{{ translate('On Delivery')}}</option>
                </select>
                <label class="my-2" >{{ translate('Order Status')}}</label>
            </div>
        </div>
    </div>
    <div class="card mt-3">
            ...
            <div class="row">
                ...
                <div class="col-lg-6">
                    <table class="details-table table">
                        ...
                        <tr>
                            <td class="w-50 strong-600">{{ translate('Order amount')}}:</td>
                        ...
                        <tr>
                            <td class="w-50 strong-600">{{ translate('Contact buyer')}}:</td>
