                                    ...
                                    <div class="col-4">
                                        <div class="sort-by-box px-1">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control sortSelect" data-placeholder="{{ translate('All City')}}" name="seller_id" onchange="filter()">
                                                    <option value="">{{ translate('All City')}}</option>
                                                    @foreach (\App\Seller::all() as $key => $seller)
                                                        @if ($seller->user != null && $seller->user->shop != null)
                                                            <option value="{{ $seller->id }}" @isset($seller_id) @if ($seller_id == $seller->id) selected @endif @endisset>{{ $seller->user->shop->city }}</option>
                                                        @endif
                                                    @endforeach
                                                </select>
                                            </div>
