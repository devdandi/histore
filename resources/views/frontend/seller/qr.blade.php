...
<div class="card">
    <div class="card-body">
        <h2>{{ translate('Transaction No #').$orderid }}</h2>
    <table class="table">
  <caption>
  <div class="float-right">
    <h5>Tax: {{ single_price($tax) }}</h5>
    <h5>Shipping: {{ single_price($shipping)}}</h5>
    <h5>Subtotal: {{ single_price($subtotal+$shipping) }}</h5>
  ...
  <thead>
    @if($orderDetail->first()->delivery_status == "pending" && $orderDetail->first()->receipt == null)
    <div class="alert alert-warning" role="alert">
        {{ translate('You have to change the status on the details order for update the receipt !') }}
    </div>
    @elseif($orderDetail->first()->delivery_status == "on_delivery" && $orderDetail->first()->receipt == null)
    <div class="alert alert-warning" role="alert">
        {{ translate('You have to put the receipt when your status is on delivery !') }}
    </div>
    @elseif($orderDetail->first()->delivery_status == "delivered" && $orderDetail->first()->receipt != null)
    <div class="alert alert-success" role="alert">
        {{ translate('You have been put the receipt for this transaction !') }}
    </div>
    @endif
  ...
  <tbody>
        ...
        @php $data = $product->where('id', $orderDetail[$i]->product_id)->first(); @endphp
            <tr>
                <td>{{ $i+1 }}</td>
                <td>{{ $data->name }}</td>
                <td>{{ $orderDetail[$i]->variation }}</td>
                <td>{{ single_price($orderDetail[$i]->price) }}</td>
                <td>{{ $orderDetail[$i]->quantity }}</td>
                <td>{{ $orderDetail[$i]->created_at }}</td>
            </tr>
        @endfor
  </tbody>
    </table>
...
<script src="{{ my_asset(mix('dist/js/app.js')) }}"></script>
<script>
